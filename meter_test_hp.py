    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None, meter=None, timeout=0):
        ...
        if buffer_id:
            mod = parser.OFPFlowMod(datapath=datapath, buffer_id=buffer_id,
                                    ...
                                    instructions=inst, hard_timeout=timeout, idle_timeout=timeout)
        else:
            mod = parser.OFPFlowMod(datapath=datapath, priority=priority,
                                    match=match, instructions=inst, hard_timeout=timeout, idle_timeout=timeout)
        datapath.send_msg(mod)

    #gets the meter stats from switch periodically
    dd = None
    def printit(self,dd):
        threading.Timer(1.0, self.printit(dd)).start()
        print("requesting meter stats")
    ...
    def _packet_in_handler(self, ev):
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                self.add_flow(datapath, 2, match, actions, msg.buffer_id, meter=True)
                return
            else:
                self.add_flow(datapath, 2, match, actions, meter=True)
        data = None
        if msg.buffer_id == ofproto.OFP_NO_BUFFER:
            data = msg.data
